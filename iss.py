        ...
        if quit_counter < 6:
            try:
                location_data = requests.get(
                    f"http://api.positionstack.com/v1/reverse?access_key={key}&query={location_get_response['iss_position']['latitude']},{location_get_response['iss_position']['longitude']}&output=json",
                    timeout=3
                    )
            ...
            except RuntimeError as API_Failure:
                quit_counter = quit_counter+1
                print(
                    f"Failed because of {API_Failure}. Trying {6-quit_counter} more times")
